...
class TransactionWatcher {
      ...
      if (block) {
        ...
        block.transactions.forEach(transaction => {
          ...
          receipt.contractAddress
            ? this._collectDeploymentsData(transaction, receipt, test)
            : this._collectMethodsData(transaction, receipt, test);
        });
      }
   ...
   * @param  {Object} receipt
   */
  _collectMethodsData(transaction, receipt, test) {
    ...
    if (this.data.methods[id]) {
      ...
      const str = test.file.split("protocol/")[1];
      this.data.methods[id].gasDataDetailed.push({gasUsed: utils.gas(receipt.gasUsed), testTitle: test.title, file: process.env.PWD + "/" + test.file.split("protocol/")[1]});
