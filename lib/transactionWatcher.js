...
class TransactionWatcher {
      ...
      if (block) {
        ...
        block.transactions.forEach(transaction => {
          ...
          receipt.contractAddress
            ...
            : this._collectMethodsData(transaction, receipt, test, isBefore);
        });
      }
   ...
   * @param  {Object} receipt
   */
  _collectMethodsData(transaction, receipt, test, isBefore) {
    ...
    if (this.data.methods[id]) {
      ...
      this.data.methods[id].gasDataDetailed.push({
        gasUsed: utils.gas(receipt.gasUsed),
        isBeforeIt: isBefore,
        testTitle: test.title,
        file: process.env.PWD + "/" + test.file.split("protocol/")[1]
      });
