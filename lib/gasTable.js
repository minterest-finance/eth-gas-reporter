  ...
  generate(info) {
    ...
    _.forEach(info.methods, (data, methodId) => {
      ...
      if (data.gasData.length) {
        ...
        stats.average = gasData[Math.ceil(gasData.length / 2)];
      ...
      if (!this.config.onlyCalledMethods || data.numberOfCalls > 0) {
        ...
        gasReport.methods[methodId] = {...info.methods[methodId], ...stats};
      }
    });
    ...
    info.deployments.forEach(contract => {
      ...
      gasReport.deployments[contract] = {...info.deployments[contract], ...stats};
    });
    ...
    const header = [
      ...
      colors.green("Avg"),
      colors.bold("# calls"),
      colors.bold(`${this.config.currency.toLowerCase()} (med)`)
    ];
    ...
    if(this.config.customGasReporterFile) {
      const report = JSON.parse(JSON.stringify(gasReport));
      for (const key in report.methods) {
        if(report.methods.hasOwnProperty(key)) {
          delete report.methods[key].gasData;

          if(!this.config.addGasUsageFromBeforeBlocks) {
            report.methods[key].gasDataDetailed = report.methods[key].gasDataDetailed.filter((data => {
              return !data.isBeforeIt;
            }));
          }
        }
      }
      fs.writeFileSync(this.config.customGasReporterFile, JSON.stringify(report), function (err) {
        if (err) return console.log(err);
        console.log(err);
      })
    } else {
      console.log("Provide file name to get json output");
    }
